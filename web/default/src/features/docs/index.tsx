...
const quickSteps: StepCard[] = [
  ...
  },
  {
    ...
    title: '优先使用一键导入',
    description: '在令牌右侧聊天菜单选择 CC Switch、Cherry Studio 等模板，减少手填错误。',
  },
  {
    icon: PlugZap,
    title: '确认 Base URL',
    description: 'Claude Code 使用站点根路径；OpenAI 兼容客户端通常使用带 /v1 的地址。',
  },
  {
    icon: CheckCircle2,
    title: '跑一次验证命令',
    description: '保存后先执行版本、模型列表或最小聊天请求，确认鉴权和模型可用。',
  },
]
    ...
    <div className='rounded-xl border bg-zinc-950 p-4 text-zinc-100'>
      <div className='flex items-start justify-between gap-3'>
        <div className='min-w-0 flex-1'>
          <pre className='overflow-x-auto whitespace-pre-wrap break-all text-sm leading-6'>
            <code>{props.code}</code>
          </pre>
          {props.note ? (
            <p className='mt-2 text-xs leading-5 text-zinc-400'>{t(props.note)}</p>
          ) : null}
        </div>
...
function CcSwitchGuide(props: {
  ...
  return (
    <GuideSection id='cc-switch' title='推荐：用 CC Switch 一键导入' icon={AppWindow}>
      ...
      <div className='grid gap-3 md:grid-cols-3'>
        {[
          ...
          <div key={step[0]} className='rounded-xl border bg-muted/40 p-4'>
            ...
            <p className='text-muted-foreground text-sm leading-6'>{t(step[2])}</p>
          </div>
        ))}
      </div>
      <div className='rounded-2xl border bg-muted/40 p-4'>
        <div className='mb-3 flex items-center justify-between rounded-xl border bg-background px-4 py-3'>
      ...
      <ol className='text-muted-foreground list-decimal space-y-2 pl-5 text-sm leading-6'>
        ...
        <li>
          {t('浏览器会拉起 CC Switch，并自动写入 Base URL 和当前令牌。')}
        </li>
...
function ClaudeGuide(props: { serverAddress: string }) {
  ...
  return (
    <div className='space-y-6'>
      ...
      <CcSwitchGuide appName='Claude' baseUrl={baseUrl} modelHint='claude-opus-4-7' />
      <GuideSection id='manual' title='备用：手动安装与配置' icon={Terminal}>
        <p className='text-muted-foreground leading-7'>
          {t('已经熟悉终端的用户可以直接按下面步骤操作。')}
        </p>
        <CodeBlock code={`node -v\nnpm -v\ngit --version`} note='三条命令都能输出版本号再继续。' />
        <CodeBlock code='npm install -g @anthropic-ai/claude-code' note='安装 Claude Code 官方 npm 包。' />
        <CodeBlock
      ...
      <GuideSection id='verify' title='验证是否跑通' icon={CheckCircle2}>
        <CodeBlock code='claude -v' note='能看到 claude-code 版本号说明安装成功。' />
        <p className='text-muted-foreground leading-7'>
          {t(
  ...
  return (
    <div className='space-y-6'>
      <div>
        ...
        <h1 className='text-3xl font-bold tracking-tight'>{t('Codex 配置手册')}</h1>
        <p className='text-muted-foreground mt-3 leading-7'>
          {t('Codex 使用 OpenAI 兼容地址，Base URL 必须带 /v1。')}
        </p>
      </div>
      <CcSwitchGuide appName='Codex' baseUrl={baseUrl} modelHint='gpt-5.3-codex' />
      <GuideSection id='codex-app' title='先下载 Codex App' icon={MonitorDown}>
        <p className='text-muted-foreground leading-7'>
          {t('OpenAI 官方 Codex App 支持桌面端。想少碰命令行，可以先安装 App；喜欢终端再补 CLI。')}
        </p>
        <div className='flex flex-wrap gap-3'>
          <Button
            variant='outline'
      ...
      <GuideSection id='cli' title='备用：Codex CLI 安装' icon={Terminal}>
        ...
        <CodeBlock code={`export OPENAI_BASE_URL="${baseUrl}"\nexport OPENAI_API_KEY="你的API_KEY"`} note='macOS / Linux 当前终端临时生效。' />
        <CodeBlock code={`$env:OPENAI_BASE_URL="${baseUrl}"\n$env:OPENAI_API_KEY="你的API_KEY"`} note='Windows PowerShell 当前窗口临时生效。' />
      </GuideSection>
      <GuideSection id='config-toml' title='config.toml 配置样例' icon={FileCode2}>
        <p className='text-muted-foreground leading-7'>
          {t('如果需要固定默认模型和服务商，可以把下面内容写入 ~/.codex/config.toml。API Key 建议放在系统环境变量里。')}
        </p>
        <CodeBlock code={configToml} />
      </GuideSection>
      <GuideSection id='verify' title='验证是否跑通' icon={CheckCircle2}>
        <CodeBlock code={`codex --version\ncodex`} note='进入项目目录后运行，选择模型或直接开始任务。' />
        <p className='text-muted-foreground leading-7'>
          {t('如果提示模型不可用，先确认当前账号分组可见 gpt-5.3-codex；如果提示网络或鉴权失败，检查 OPENAI_BASE_URL 是否带 /v1。')}
        </p>
      </GuideSection>
    </div>
  )
  ...
  return (
    <div className='space-y-6'>
      <div>
        ...
        <h1 className='text-3xl font-bold tracking-tight'>{t('OpenClaw 配置手册')}</h1>
        <p className='text-muted-foreground mt-3 leading-7'>
          {t('OpenClaw 接本站时走 OpenAI 兼容接口。最容易填错的是模型名前缀，默认模型建议写成 newapi/模型ID。')}
        </p>
      </div>
      <GuideSection id='quick-copy' title='复制给 OpenClaw 的配置说明' icon={Copy}>
        <div className='grid gap-3 md:grid-cols-2'>
      ...
      <GuideSection id='setup' title='手动接入步骤' icon={Workflow}>
        <p className='text-muted-foreground leading-7'>
          {t('先在本站创建或复制令牌，再运行 OpenClaw 的接入向导。模型 ID 可以在模型广场查看，或通过 /v1/models 接口查询。')}
        </p>
        <CodeBlock code='npm install -g openclaw@latest' note='已安装过 OpenClaw 的用户可跳过。' />
        <CodeBlock code={`export NEWAPI_API_KEY="你的API_KEY"\nopenclaw onboard`} note='Windows PowerShell 使用 $env:NEWAPI_API_KEY="你的API_KEY"。' />
        <ol className='text-muted-foreground list-decimal space-y-2 pl-5 text-sm leading-6'>
      ...
      <GuideSection id='verify' title='验证是否跑通' icon={CheckCircle2}>
        ...
        <p className='text-muted-foreground leading-7'>
          {t('如果模型列表为空，先检查 API Key 是否属于当前站点令牌；如果模型调用报错，优先检查默认模型是否带了 newapi/ 前缀。')}
        </p>
      </GuideSection>
    </div>
  )
  ...
  return (
    <div className='space-y-6'>
      <div>
        ...
        <h1 className='text-3xl font-bold tracking-tight'>{t('Hermes 配置手册')}</h1>
        <p className='text-muted-foreground mt-3 leading-7'>
          {t('Hermes Agent 支持自定义 OpenAI 兼容服务。本站配置只需要 Base URL、API Key 和模型 ID。')}
        </p>
      </div>
      <GuideSection id='fields' title='推荐填写项' icon={KeyRound}>
        <div className='grid gap-3 md:grid-cols-2'>
          <FieldCard label='Provider' value='Custom endpoint / OpenAI-compatible' />
          <FieldCard label='Base URL' value={baseUrl} />
          <FieldCard label='API Key' value='本站令牌 sk-...' />
          <FieldCard label='Model' value='gpt-5.3-codex' />
        </div>
        <p className='text-muted-foreground text-sm leading-6'>
          {t('如果账号分组没有 gpt-5.3-codex，可在模型广场选择当前可用模型，把模型 ID 原样填进去。')}
        </p>
      </GuideSection>
      <GuideSection id='interactive' title='交互式配置' icon={Terminal}>
        <CodeBlock code={`export NEWAPI_API_KEY="你的API_KEY"\nhermes model`} note='Windows PowerShell 使用 $env:NEWAPI_API_KEY="你的API_KEY"。' />
      </GuideSection>
      <GuideSection id='config-yaml' title='config.yaml 写法' icon={FileCode2}>
        <p className='text-muted-foreground leading-7'>
          {t('如果想手动维护配置，可以把下面内容写入 Hermes 配置文件，再把令牌放到环境变量或 .env 中。')}
        </p>
      ...
      <GuideSection id='verify' title='验证是否跑通' icon={CheckCircle2}>
        ...
        <p className='text-muted-foreground leading-7'>
          {t('如果提示 401，检查 API Key；如果提示 404 或模型不存在，检查 Base URL 是否带 /v1，以及模型 ID 是否属于当前账号可用分组。')}
        </p>
      </GuideSection>
    </div>
  )
...
function CommonSetup(props: { serverAddress: string }) {
  ...
  const fields: Field[] = [
    ...
    },
    {
      ...
      helper: '用于 Codex、OpenClaw、Hermes、Cherry Studio 等 OpenAI-compatible 客户端。',
    },
    {
      label: 'API Key',
      value: 'sk-... 你的本站令牌',
...
export function Docs(props: DocsProps) {
  ...
  const selectedGuide = guides.find((guide) => guide.slug === selectedSlug) ?? guides[0]
          ...
          <aside className='lg:sticky lg:top-20 lg:self-start'>
            <div className='rounded-xl border p-4'>
              ...
              <div className='space-y-2'>
                <p className='text-muted-foreground text-xs'>{t('聊天应用模板')}</p>
          ...
          <div className='space-y-8'>
            <Card>
              <CardHeader>
                ...
                <CardDescription>{t(selectedGuide.description)}</CardDescription>
              </CardHeader>
            </Card>
