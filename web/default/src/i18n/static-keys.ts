...
export const STATIC_I18N_KEYS = [
  ...
  'Start using AI models in minutes',
  'Follow this guide to create an API key, connect your favorite client, and verify your first request without reading scattered external documents.',
  'Open Console',
  'View Model Square',
  'Create an account',
  'Sign in to the console and complete any required account verification.',
  'Add balance',
  'Top up your account or redeem a code before sending paid requests.',
  'Create an API key',
  'Open API Keys, create a key, and copy it once. Keep it private.',
  'Connect your client',
  'Use the API endpoint and your key in any OpenAI-compatible client.',
  'On this page',
  'Account preparation',
  'API key setup',
  ...
  'Safety checklist',
  'Before connecting a client, make sure your account can send requests.',
  'Register or sign in to your account.',
  'Check whether your account has remaining quota.',
  'Confirm the model you want to use is available in Model Square.',
  'Create a dedicated key for each app or client you connect.',
  'Open Console from the top navigation.',
  'Go to API Keys and click Create API Key.',
  'Set a clear name, optional quota limit, and allowed models if needed.',
  'Copy the key immediately and store it in a secure place.',
  'Recommended key format',
  'Most OpenAI-compatible clients only need three fields: endpoint, key, and model.',
  'API Endpoint',
  'Use your site domain and keep the /v1 suffix.',
  'API Key',
  'Paste the key created in Console → API Keys.',
  'Model',
  'Select any model available in Model Square.',
  'Different models may use different billing ratios.',
  'Common clients',
  'Desktop client with OpenAI-compatible custom provider support.',
  'Lightweight desktop client for quick model testing.',
  'Mobile and desktop client for everyday chat workflows.',
  'Web client that can connect to custom OpenAI-compatible endpoints.',
  'Send one small request after saving the client settings.',
  'If the request fails, check whether the endpoint includes /v1, the key is active, the selected model is enabled, and your account has enough quota.',
  'Use these habits to keep your account and quota safe.',
  'Never share API keys in screenshots, public repositories, or browser extensions you do not trust.',
  'Create separate keys for different clients so you can revoke one without affecting others.',
  'Set quota limits for experimental keys to avoid unexpected usage.',
